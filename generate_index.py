...
#playerTitle{{position:absolute;top:20px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.7);padding:8px 14px;border-radius:8px;font-size:18px;font-weight:bold;opacity:0;transition:opacity 1s;}}
...
function showTitle(text) {{
    ...
    titleTimeout=setTimeout(()=>{{ playerTitle.style.opacity=0; }},5000);
}}
...
// Se tocchi/clicki lo schermo, riappare il titolo
overlay.addEventListener('click', ()=>{{
    if(playerTitle.textContent.trim()!==''){{
        showTitle(playerTitle.textContent);
    }}
}});
</script>
</body>
