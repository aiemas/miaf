...
/* Chiudi la scheda info e vai alla griglia */
...
function openInfo(item, push=true) {{
    ...
    // CHIUDI SEMPRE IL PLAYER quando apri la scheda info (importante per il tasto indietro)
...
function openPlayer(item, push=true) {{
    currentItem = item;
    // salva se la card info era visibile, così la riapriamo alla chiusura
    overlay.dataset.prevCardVisible = infoCard.style.display === 'block' ? 'true' : 'false';

    // nascondi info e mostra player
    infoCard.style.display = 'none';
...
/* Gestione della navigazione indietro/avanti - mappa lo stato e applica la UI correttamente */
window.addEventListener("popstate", function(e) {{
    const state = e.state;

    if(!state || state.page==="grid") {{
        // stato iniziale o griglia: chiudi tutto
        overlay.style.display='none';
        iframe.src='';
        infoCard.style.display='none';
        return;
    ...
    if(state.page === "player") {{
        if(item) {{
            openPlayer(item, false); // applica stato senza pushare di nuovo
        }} else {{
            // fallback: mostra solo il player vuoto
            overlay.style.display='flex';
        }}
    }} else if(state.page === "info") {{
        if(item) {{
            openInfo(item, false); // applica stato senza pushare di nuovo
        }} else {{
            // fallback: chiudi tutto
            overlay.style.display='none';
            iframe.src='';
            infoCard.style.display='none';
        }}
...
/* Imposto stato iniziale per poter navigare con back/forward */
...
def main():
    ...
    for type_, url in SRC_URLS.items():
        ...
        for idx, tmdb_id in enumerate(ids):
            ...
            # Aggiorna storico (salvo l'intero entry così poi puoi decidere cosa mostrare)
