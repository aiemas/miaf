...
# ... [tutto il codice precedente rimane identico fino a build_html]
...
<title>Movies & Series</title>
<style>
/* ... [tutto lo stile precedente rimane identico] ... */
#overlayTitle {{
    position: absolute;
    top: 20px;
    left: 20px;
...
/* ... [showLatest e openInfo rimangono identici] ... */

function openPlayer(item){{ 
    infoCard.style.display = 'none';
    overlay.style.display='flex';

    // Mostra titolo in sovraimpressione
    if(item.type==='tv'){{
        let season = parseInt(seasonSelect.value) || 1;
        let episode = parseInt(episodeSelect.value) || 1;
        overlayTitle.textContent = `${{item.title}} - S${{season}}E${{episode}}`;
    }} else {{
