...
- Click su locandina apre scheda informativa in overlay (titolo, descrizione, voto, generi, pulsante Play)
...
.badge{{position:absolute;bottom:8px;right:8px;padding:3px 6px;font-size:13px;font-weight:bold;border-radius:6px;}}
.badge.green{{background:#21d07a;color:#000;}}
...
/* Overlay info */
#infoOverlay{{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);display:none;align-items:center;justify-content:center;z-index:2000;}}
#infoContent{{background:#111;padding:20px;border-radius:10px;max-width:600px;width:90%;color:#fff;position:relative;}}
#infoContent img{{width:150px;float:left;margin-right:20px;border-radius:6px;}}
#infoContent h2{{margin-top:0;}}
#infoContent .genres{{font-size:14px;color:#aaa;margin-bottom:10px;}}
#infoContent .overview{{clear:both;margin-top:10px;font-size:15px;line-height:1.4;}}
#infoContent .playBtn{{margin-top:15px;padding:10px 20px;background:#e50914;color:#fff;border:none;border-radius:5px;cursor:pointer;font-size:16px;}}
#infoContent .closeBtn{{position:absolute;top:10px;right:15px;font-size:25px;background:transparent;border:none;color:#fff;cursor:pointer;}}
/* Player overlay */
#playerOverlay{{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);display:none;align-items:center;justify-content:center;z-index:3000;flex-direction:column;}}
...
function openInfo(item){{
    ...
    if(item.vote < 5) badgeClass='red'; else if(item.vote < 7) badgeClass='yellow';
    infoDetails.innerHTML = `
        <img src="${{item.poster}}" alt="${{item.title}}">
        <h2>${{item.title}}</h2>
        <div class="genres">${{item.genres.join(', ')}}</div>
        <div class="badge ${{badgeClass}}">★ ${{item.vote}}</div>
        <div class="overview">${{item.overview || "Nessuna descrizione disponibile."}}</div>
    `;
}}
...
function render(reset=false){{
    ...
    while(shown < currentList.length && count < step){{
        ...
        if((g==='all' || m.genres.includes(g)) && m.title.toLowerCase().includes(s)){{
            let badgeClass = 'green';
            if(m.vote < 5) badgeClass='red'; else if(m.vote < 7) badgeClass='yellow';
